...
- answer packets are cited, deterministic, freshness-aware, gap-aware,
  chain-aware, and synthesis-aware: weak evidence returns an explicit
  insufficient-evidence report instead of a fabricated snippet answer, while
  grounded answers carry `agent_answer`, `evidence_chain`, and `nuance_report`
