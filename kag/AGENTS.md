...
This card applies to `aoa-4pda-connector/kag/` and every nested path until a
nearer card narrows the lane.

## Role

`kag/` is the local KAG provider home for `aoa-4pda-connector`.

It exposes compact source-linked records over the 4PDA connector policy,
storage boundary, and runtime contract surfaces for `aoa-kag` registry,
...
Keep 4PDA connector meaning with this repository's source surfaces. Keep shared
