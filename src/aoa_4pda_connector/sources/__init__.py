"""Operator-local 4PDA source registry and bounded crawl planning."""
...
SOURCE_KINDS = {"topic", "forum_section", "profile_seed", "discovery_candidate"}
ACCESS_MODES = {"public", "account_visible"}
MEDIA_POLICIES = {"none", "thumbnails", "documents", "all"}
REGISTRY_SCHEMA = "aoa_4pda_source_registry_v1"
SOURCE_SCHEMA = "aoa_4pda_source_v1"
PLAN_SCHEMA = "aoa_4pda_source_crawl_plan_v1"


def registry_path(data_root: Path) -> Path:
    return data_root / "sources" / "4pda_sources.json"
...
def load_registry(data_root: Path) -> dict[str, object]:
    ...
    if registry.get("schema") != REGISTRY_SCHEMA:
        raise ValueError(f"unsupported 4PDA source registry schema: {registry.get('schema')}")
    return registry
...
def build_source_plan(*, run_id: str, sources: list[dict[str, object]], max_pages: int, include_media: str | None = None) -> dict[str, object]:
    steps = []
...
def _validate(source_ref: str, kind: str, access: str, include_media: str, trust_score: float | None) -> None:
    ...
    if kind not in SOURCE_KINDS:
        raise ValueError(f"unsupported 4PDA source kind: {kind}")
    if access not in ACCESS_MODES:
        raise ValueError(f"unsupported 4PDA access mode: {access}")
